...
router.post('/register', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // birthday is in format 2021-01-01
    // change to 01/01/2021
    const birthdayArray = birthday.split('-');
    const birthdayString = birthdayArray[1] + '/' + birthdayArray[2] + '/' + birthdayArray[0];
    const user = await createUserByEmailAndPassword(email, password, name, LastName, birthdayString);
    const userId = user.id;
    const jti = v4();
    const { accessToken, refreshToken } = generateTokens(user, jti);
    await addRefreshTokenToWhitelist({ jti, refreshToken, userId: user.id });
